...
export function APCAonly(
  ...
  var Ybg =
    Math.pow(BGr / 255.0, sRGBtrc) * Rco +
    Math.pow(BGg / 255.0, sRGBtrc) * Gco +
    Math.pow(BGb / 255.0, sRGBtrc) * Bco;

  var Ytxt =
    Math.pow(txtr / 255.0, sRGBtrc) * Rco +
  ...
  if (Ybg >= Ytxt) {
    ...
    if (Ytxt > Ybg) {
      ...
      outputContrast =
        (Math.pow(Ybg, normBGExp) - Math.pow(Ytxt, normTXTExp)) * scaleBoW;
    }
  } else {
    // Same, but reverse polarity (white text on black)
    ...
    if (Ybg > Ytxt) {
      ...
      outputContrast =
        (Math.pow(Ybg, revBGExp) - Math.pow(Ytxt, revTXTExp)) * scaleWoB;
...
//  */
  ...
  return outputContrast > clipLevel || outputContrast < -clipLevel
    ? (outputContrast * 100).toFixed(1) + " Lc"
    : polarity + "LOW";
}
